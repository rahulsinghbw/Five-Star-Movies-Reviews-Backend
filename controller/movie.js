...
exports.updateMovie = async (req, res) => {
  ...
  if (file) {
    ...
    // Now here we upload a new poster to cloudinary
...
exports.removeMovie = async (req, res) => {
  const { movieId } = req.params;
  if (!isValidObjectId(movieId)) return sendError(res, "Invalie Movie ID!");

  const movie = await Movie.findById(movieId);
  if (!movie) return sendError(res, "Movie Not Found!", 404);

  // check if there is poster is not
  // if yes then we need to remove that poster
  const posterId = movie.poster?.public_id;
  if (posterId) {
    const { result } = await cloudinary.uploader.destroy(posterId);
    if (result !== "ok")
  ...
  // Now we removing the trailer from cloud
...
exports.getMovies = async (req, res) => {
  ...
  const results = movies.map((movie) => ({
    id: movie._id,
    title: movie.title,
    poster: movie.poster?.url, // If there is no poster then we don't want to genereate any error so That's why we use "?"
...
exports.getSingleMovie = async (req, res) => {
  ...
  //Pipeline is the set of operations
